...
class MediaPlayer(Frame):
    def __init__(self, master=None, path=None):
        ...
        mediaPlayButton = Button(self.playMediaWindow, image=self.playimage, width=160, command=lambda : subprocess.call(['audtool', '--playback-play']))
        mediaPlayButton.place(x=10, y=10)
        mediaPauseButton = Button(self.playMediaWindow, image=self.pauseimage, width=160, command=lambda : subprocess.call(['audtool', '--playback-pause']))
        mediaPauseButton.place(x=180, y=10)
        mediaStopButton = Button(self.playMediaWindow, image=self.stopimage, width=160, command=lambda : subprocess.call(['audtool', '--playback-stop']))
        mediaStopButton.place(x=350, y=10)
        mediaPrevButton = Button(self.playMediaWindow, image=self.previmage, width=160, command=lambda : subprocess.call(['audtool', '--playlist-advance']))
